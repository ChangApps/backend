...
class HistorialSolicitudesViewSet(viewsets.ViewSet):
    ...
    def get_solicitudes_proveedor(self, request, *args, **kwargs):
        ...
        # obtiene el historial (solicitudes donde soy proveedor)
        ...
        # IDs de usuarios que ME bloquearon
        ids_que_me_bloquearon = list(
            Usuario.objects.filter(bloqueados=usuario).values_list('id', flat=True)
        )

        # Excluir solicitudes cuyo cliente está bloqueado por mí
        if ids_bloqueados_por_mi:
            historial = historial.exclude(cliente_id__in=ids_bloqueados_por_mi)

        # Excluir solicitudes cuyo cliente me bloqueó a mí
        if ids_que_me_bloquearon:
            historial = historial.exclude(cliente_id__in=ids_que_me_bloquearon)

        historial = historial.distinct()
